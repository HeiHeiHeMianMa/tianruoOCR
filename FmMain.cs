...
//                                   "&vol=9&per=0&spd=6&pit=4&source=web&ctp=1";
		...
		public void OCR_baidu()
		{
			...
			try
			{
				baidu_vip = CommonHelper.GetHtmlContent("https://aip.baidubce.com/oauth/2.0/token?grant_type=client_credentials&client_id=" + IniHelper.GetValue("密钥_百度", "secret_id") + "&client_secret=" + IniHelper.GetValue("密钥_百度", "secret_key"));
...
//                        "&to=", text3));
				...
				var jArray = JArray.Parse(((JObject)JsonConvert.DeserializeObject(html))["trans_result"].ToString());
				foreach (var arr in jArray)
                {
                    text = text + arr["dst"] + "\r\n";
                }
